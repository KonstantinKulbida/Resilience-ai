...
export default async function handler(req: any, res: any) {
  ...
  const responseLanguage = language === 'ru' ? 'ru' : 'en';
  ...
  const defaultIds = getDefaultActionIds(scores.weakestFactor);

  let insight = buildFallbackInsight(scores, responseLanguage);
  let selectedIds = defaultIds;
  let aiEnhanced = false;

  try {
    const personalization = await generateAssessmentPersonalization(
      scores,
      responseLanguage
    );
    ...
    selectedIds = {
      ...
      support: personalization.supportActionId,
    };
    aiEnhanced = true;
  } catch (error) {
    console.error('Assessment AI personalization error:', error);
  }
  ...
  return res.status(200).json({
    ...
    insight,
    actions: {
      today: resolveAction(selectedIds.today, responseLanguage),
      week: resolveAction(selectedIds.week, responseLanguage),
      support: resolveAction(selectedIds.support, responseLanguage),
    },
    aiEnhanced,
  });
}
