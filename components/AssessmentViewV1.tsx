...
const AssessmentViewV1: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // The app scrolls inside its main content container, not the browser window.
  ...
  useEffect(() => {
    ...
    const target =
      resultStep === 2 ? driversRef : resultStep === 3 ? insightRef : actionsRef;
    const frame = window.requestAnimationFrame(() => {
  ...
  const submitAssessment = async () => {
    if (Object.keys(answers).length < questions.length) {
      setError(t('Please answer all questions.', 'Пожалуйста, ответьте на все вопросы.'));
      return;
    }

    setLoading(true);
    setError(null);
    const nextResult = await analyzeAssessment(answers as Record<string, number>, language);
    setLoading(false);
  ...
  if (!result) {
    return (
      <div ref={topRef} className="max-w-4xl mx-auto pb-10 animate-enter">
        <section className="bg-white/50 backdrop-blur-xl rounded-[2rem] p-5 sm:p-8 border border-white/60 shadow-lg shadow-indigo-500/5">
          <div className="mb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2">
              {t('Work Sustainability assessment', 'Оценка устойчивости рабочего режима')}
            </h2>
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white/70 p-4 text-sm text-slate-600">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <span>
                  <strong className="text-slate-900">1</strong> — {t('Not at all true', 'Совсем не похоже на меня')}
                </span>
                <span>
                  <strong className="text-slate-900">5</strong> — {t('Very true', 'Полностью похоже на меня')}
                </span>
              </div>
          ...
          >
            {loading
              ? t('Calculating your result…', 'Рассчитываем ваш результат…')
  ...
  return (
    <div ref={topRef} className="max-w-6xl mx-auto space-y-6 sm:space-y-8 pb-10 animate-enter">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <p className="text-sm text-slate-500">
            {t('Assessment result', 'Результат оценки')} · {new Date().toLocaleDateString(locale)}
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="bg-white/60 backdrop-blur-xl text-slate-900 rounded-[2.5rem] p-6 sm:p-10 border border-white/75 shadow-xl shadow-indigo-500/10 overflow-hidden relative">
        ...
        <div className="relative grid lg:grid-cols-[1fr_auto] gap-8 lg:items-end">
          <div>
            <p className="text-sm font-semibold text-slate-500 mb-3">
              {t('Your Work Sustainability', 'Насколько устойчив ваш рабочий режим')}
            </p>
            <div className="flex flex-wrap items-end gap-4 mb-4">
              <span className="text-6xl sm:text-7xl font-bold tracking-tight text-slate-950">{result.score}</span>
        ...
        <div className="relative mt-6 pt-5 border-t border-slate-200/80 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button
            ...
            className="self-start sm:self-auto inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-slate-900/10 transition hover:bg-slate-800"
          >
            {t('See what shapes your score', 'Посмотреть, из чего складывается результат')}
      ...
      <section ref={driversRef} className="scroll-mt-6">
        <div className="mb-4">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900">
            {t("What’s shaping your score", 'Что влияет на ваш результат')}
          </h2>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-5">
                  <div>
                    <h3 className="font-bold text-slate-900">{factorLabel(factor)}</h3>
                    {factor === displayWeakestFactor && (
                      <span className="mt-1.5 inline-flex rounded-full bg-rose-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-rose-700 border border-rose-100">
                        {t('Main pressure point', 'Главная зона напряжения')}
                      </span>
                    )}
                  </div>
                  <span
                ...
                <div className="flex items-end gap-2 mb-3">
                  <span className="text-4xl font-bold text-slate-900">{factorResult.score}</span>
        ...
        <div className="mt-5 flex justify-end">
          ...
          >
            {t('What matters most right now', 'Что сейчас важнее всего')}
      ...
      {resultStep >= 3 && (
      <section ref={insightRef} className="scroll-mt-6 bg-indigo-50/70 border border-indigo-100 rounded-[2rem] p-5 sm:p-7">
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-indigo-600 mb-1">
              {t('Personalized insight', 'Персональный вывод')}
            </p>
            <h2 className="text-lg sm:text-xl font-bold text-slate-900 mb-2">
              {t('What matters most right now', 'Что сейчас важнее всего')}
            </h2>
            <p className="text-slate-700 leading-relaxed max-w-4xl">{result.insight}</p>
            <button
              type="button"
            ...
            >
              {t('See what you can do', 'Посмотреть, что можно сделать')}
              <ArrowDown className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      {resultStep >= 4 && (
      <>
      <section ref={actionsRef} className="scroll-mt-6">
        <div className="mb-4">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900">
            {t('What to do next', 'Что делать дальше')}
          </h2>
          <p className="text-slate-500 mt-1">
            {t(
              `Your lowest-scoring factor is ${factorLabel(displayWeakestFactor)}. Start there.`,
              `Самый низкий результат сейчас у фактора «${factorLabel(displayWeakestFactor)}». Начните с него.`
            )}
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-4 sm:gap-6">
          {[
            ...
            {
              label: t('This week', 'На этой неделе'),
              action: result.actions.week,
              icon: RefreshCw,
            },
            {
              label: t('Get support', 'Получить поддержку'),
      ...
      <section className="bg-white/55 backdrop-blur-xl rounded-[2rem] p-5 sm:p-7 border border-white/70 shadow-sm">
        <div className="flex items-start gap-4">
          ...
          <div className="max-w-4xl">
            <h2 className="text-lg sm:text-xl font-bold text-slate-900 mb-2">
              {t('Your privacy', 'Ваша конфиденциальность')}
            </h2>
            <p className="font-semibold text-slate-800 mb-3">
              {t('Your individual result is private to you.', 'Ваш индивидуальный результат видите только вы.')}
            </p>
